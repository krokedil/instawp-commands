...
wp import --url="https://raw.githubusercontent.com/krokedil/instawp-commands/refs/heads/main/assets/wxr-exports/products-coupons-for-tests.wordpress.xml" --authors=skip
...
wp user meta add 1 billing_country "SE"
...
wp user meta add 1 shipping_country "SE"
...
wp post delete $(wp post list --pagename='sample-page' --format=ids) --force # Delete defualt page "Sample page"
wp post delete $(wp post list --post_name='hello-world' --format=ids) --force # Delete defualt post "Hello world!"
wp post update $(wp post list --pagename='privacy-policy' --format=ids) --post_status=publish
wp post update $(wp post list --pagename='refund_returns' --format=ids) --post_status=publish
